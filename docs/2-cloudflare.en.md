...
## 1. status × cf-mitigated — how to read a response
...
- **`429` + (none)** — true REST API quota reached. Fix in Section 5.
...
- **`403` + `bot`** — Bot Management triggered (Enterprise / Super Bot Fight Mode).
...
| `cf-mitigated` | See Section 1 | Various WAF / quota scenarios |
